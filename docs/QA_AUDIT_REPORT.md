...
| 8 | Quality / Tests | 15% | 87 | 324 Flutter tests pass, analyze clean, no crashes. **New:** 49 mocked-Admin-SDK Cloud Functions unit tests + 7 tests run against a real Firestore/Functions emulator (first backend test coverage in the repo) — still not wired into CI (see L-6). |
...
- **Automated quality:** **324 Flutter tests pass** plus **56 new Cloud Functions tests** (49 mocked-unit + 7 real-emulator integration), `flutter analyze` clean bar 4 infos, **no crashes/ANRs** across the whole live session.
